...
class FetchIndianNewsListWorkManager @Inject constructor(
    ...
    override suspend fun doWorker(): Result {
        ...
        source?.let { sourceVal ->
            val response = newsApiService.getIndianNews(sourceVal)

            if (response.isSuccessful) {
                if (response.body() != null) {
                    response.body()!!.listOfIndianNewsDetail?.let { newsDao.upsertAllIndianNewsDetails(it) }
...
class FetchIndianNewsListWorkFactory @Inject constructor(
    private val newsApiService: NewsApiService,
    private val newsDao: NewsDao
): CustomWorkerFactory {
    override fun create(context: Context, workerParameters: WorkerParameters): ListenableWorker {
        return FetchIndianNewsListWorkManager(context, workerParameters, newsApiService, newsDao)
    }
